...
int main ()
{
    static REAL aa[3000][3000],a[3000][3001],b[3000],x[3000];
    REAL cray,ops,total=0.0,norma,normx;
    REAL resid=0.0,residn=0.0,eps=0.0,t1,t2;
    REAL epslon(),kf;
    static int ipvt[3000],n,i,ntimes,info,lda,ldaa,kflops;
...
/*     compute a residual to verify results.  */
    ...
    printf("      dgefa      dgesl      total       kflops     unit      ratio\n");
    ...
    atime[0][3] = (second() - t1 - t2)/ntimes;

    t1 = second();
    for (i = 0; i < ntimes; i++) {
                dgesl(a,lda,n,ipvt,b,0);
    }
    atime[1][3] = (second() - t1)/ntimes;

    for(i=1;i<=3;++i){
        total = atime[0][i] + atime[1][i];
        atime[2][i] = total;
        atime[3][i] = ops/(1.0e3*total);
        atime[4][i] = 2.0e3/atime[3][i];
        atime[5][i] = total/cray;
    }
    ...
    atime[0][7] = (second() - t1 - t2)/ntimes;

    t1 = second();
    for (i = 0; i < ntimes; i++) {
        dgesl(aa,ldaa,n,ipvt,b,0);
    }
    atime[1][7] = (second() - t1)/ntimes;
